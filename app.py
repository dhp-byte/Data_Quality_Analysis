...
Structure :
    ...
    src/survey_client.py   -> client API Survey Solutions
    src/quality_checks.py  -> calcul des indicateurs de qualité
    src/scoring.py         -> calcul du score par enquêteur
    src/demo_data.py       -> données de démonstration
    src/ui_style.py        -> habillage visuel (CSS)
"""
...
# ----------------------------------------------------------------------
with st.sidebar:
    st.header("🔐 Connexion au serveur")

    data_source = st.radio(
        "Source des données",
        options=["Démonstration (sans serveur)", "Serveur Survey Solutions"],
        index=0,
        help="Choisissez 'Démonstration' pour tester l'application immédiatement avec des données simulées.",
    )

    if data_source == "Serveur Survey Solutions":
        server_url = st.text_input("URL du serveur", placeholder="https://survey.monorganisation.org")
        auth_mode = st.radio("Authentification", ["Identifiant / mot de passe", "Jeton API (Bearer)"], horizontal=False)

        username = password = token = None
        if auth_mode == "Identifiant / mot de passe":
            username = st.text_input("Identifiant API")
            password = st.text_input("Mot de passe", type="password")
        else:
            token = st.text_input("Jeton API", type="password")
        ...
        if connect_clicked:
            if not server_url:
                st.error("Veuillez renseigner l'URL du serveur.")
            elif auth_mode == "Identifiant / mot de passe" and (not username or not password):
                st.error("Veuillez renseigner l'identifiant et le mot de passe.")
            elif auth_mode == "Jeton API (Bearer)" and not token:
                st.error("Veuillez renseigner le jeton API.")
            else:
                creds = SurveyCredentials(
                    server_url=server_url,
                    workspace=workspace or "primary",
                    auth_mode="token" if auth_mode == "Jeton API (Bearer)" else "basic",
                    username=username,
                    password=password,
                    token=token,
                )
        ...
        if st.session_state.client:
            ...
            try:
                ...
                if quest_df.empty:
                    ...
                    label_col = "Title" if "Title" in quest_df.columns else quest_df.columns[0]
                    id_col = "QuestionnaireId" if "QuestionnaireId" in quest_df.columns else quest_df.columns[0]
                    version_col = "Version" if "Version" in quest_df.columns else None

                    choice = st.selectbox("Sélectionner un questionnaire", quest_df[label_col].tolist())
                    selected_row = quest_df[quest_df[label_col] == choice].iloc[0]
                    q_id = selected_row[id_col]
                    q_version = int(selected_row[version_col]) if version_col else 1

                    if st.button("📥 Charger les interviews", type="primary", use_container_width=True):
                        with st.spinner("Récupération des interviews..."):
                            try:
                                interviews = st.session_state.client.list_interviews(q_id, q_version)
        ...
        st.info("Mode démonstration actif : données simulées pour 10 enquêteurs.")
    ...
    st.divider()
    st.subheader("⚖️ Fichier local (option)")
    uploaded = st.file_uploader("Ou importer un export CSV/Excel", type=["csv", "xlsx"])
    ...
    st.caption("Ajustez le poids de chaque critère dans le score de qualité (somme non contrainte, renormalisée automatiquement).")
    weights = {}
    ...
    weights["outlier_rate"] = st.slider("Valeurs aberrantes", 0, 50, DEFAULT_WEIGHTS["outlier_rate"])
...
if st.session_state.raw_df is None:
    st.info(
        "👈 Connectez-vous à votre serveur Survey Solutions, générez des données "
        "de démonstration, ou importez un fichier CSV/Excel depuis la barre latérale pour commencer."
    )
    st.stop()
...
with tabs[0]:
    ...
    completion_rate = 1 - df["missing_rate"].mean() if "missing_rate" in df.columns and df["missing_rate"].notna().any() else np.nan
    ...
    comp_display = f"{completion_rate*100:.1f}%" if pd.notna(completion_rate) else "N/A"
    c4.markdown(kpi_card("Taux de complétude", comp_display), unsafe_allow_html=True)
    ...
    with col2:
        ...
        if "status" in df.columns:
            ...
            fig2 = px.pie(status_counts, names="status", values="count", hole=0.55,
                          color_discrete_sequence=["#4ADE80", "#F87171", "#FCD34D", "#60A5FA"])
...
with tabs[1]:
    ...
    st.caption("Score composite (0–100) basé sur les taux d'anomalies pondérés — voir réglages dans la barre latérale.")
    ...
    with col_table:
        st.dataframe(
            display_df[[
                ...
                "taux_anomalies_gps": "% Anomalies GPS", "taux_aberrants": "% Aberrants",
                "taux_rejet": "% Rejetées",
            }),
            use_container_width=True, hide_index=True,
            column_config={
                "Score": st.column_config.ProgressColumn("Score", min_value=0, max_value=100, format="%.0f"),
                "% Manquants": st.column_config.NumberColumn(format="%.1%"),
                ...
                "% Aberrants": st.column_config.NumberColumn(format="%.1%"),
    ...
    with col_detail:
        ...
        radar_categories = ["Manquants", "Doublons", "Durée anormale", "Anomalies GPS", "Aberrants", "Rejet"]
        radar_values = [
...
with tabs[3]:
    ...
    st.divider()
    st.subheader("⚠️ Valeurs aberrantes")
    if "n_outliers" in df.columns:
        ...
        fig4.update_layout(paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
                           font_color="#FAFAFA", height=400, xaxis_title="Enquêteur", yaxis_title="Nb. valeurs aberrantes")
...
with tabs[4]:
    ...
    if "duration_minutes" in df.columns:
        ...
        st.warning("Colonne de durée non disponible.")
...
with tabs[5]:
    ...
    if "latitude" in df.columns and "longitude" in df.columns:
        ...
        st.warning("Coordonnées GPS non disponibles dans ce jeu de données.")
